...
class JobKoreaScraper(BaseScraper):
    ...
    def search(self) -> list[dict]:
        ...
        for keyword in self.keywords:
            try:
                ...
                logger.error(f"잡코리아 검색 오류 (키워드: {keyword}): {e}")
    ...
    def _search_keyword(self, keyword: str) -> list[dict]:
        ...
        for page in range(1, 4):  # 최대 3페이지
            ...
            if not job_list:
                # 다른 선택자 시도
                job_list = soup.select("div.list-default article")
    ...
    def _parse_job_element(self, elem) -> Optional[dict]:
        ...
        try:
            # 제목과 링크
            title_elem = elem.select_one("a.information-title-link")
            if not title_elem:
            ...
            # 근무지
            location_elem = elem.select_one("span.loc")
            if not location_elem:
                location_elem = elem.select_one("p.option span:first-child")
            location = location_elem.get_text(strip=True) if location_elem else ""

            # 경력
            exp_elem = elem.select_one("span.exp")
            if not exp_elem:
                exp_elem = elem.select_one("p.option span:nth-child(2)")
