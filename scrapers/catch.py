...
class CatchScraper(BaseScraper):
    ...
    SEARCH_URL = "https://www.catch.co.kr/Comp/Recruit/RecruitList"
    ...
    def search(self) -> list[dict]:
        ...
        for keyword in self.keywords:
            try:
                ...
                logger.error(f"캐치 검색 오류 (키워드: {keyword}): {e}")
    ...
    def _search_keyword(self, keyword: str) -> list[dict]:
        ...
        for page in range(1, 4):  # 최대 3페이지
            ...
            # 채용공고 목록 찾기
            job_list = soup.select("div.recruit_list li")
            if not job_list:
                job_list = soup.select("ul.list_recruit li")
    ...
    def _parse_job_element(self, elem) -> Optional[dict]:
        ...
        try:
            # 제목과 링크
            title_elem = elem.select_one("a.tit")
            if not title_elem:
                title_elem = elem.select_one("strong.tit a")
            if not title_elem:
            ...
            # 회사명
            company_elem = elem.select_one("span.corp_name")
            if not company_elem:
                company_elem = elem.select_one("a.company")
            if not company_elem:
            ...
            if not exp_elem:
                exp_elem = elem.select_one("span.exp")
